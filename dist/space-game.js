...
function finish(){state='over';keys={};cancelAnimationFrame(raf);if(game.time>best){best=game.time;safeStore.set('vibecode-space-best',String(best));$('#best').textContent=best.toFixed(1);}$('#pause-game').disabled=true;$('#difficulty').disabled=false;$('#shield-option').disabled=false;updateHUD();showOverlay('FIM DE PARTIDA',`${game.time.toFixed(1)} segundos`,`${game.kills} alvos destruídos · Nível ${game.level}. Prepare sua próxima missão!`,'Jogar novamente');$('#start-game').focus({preventScroll:true});}
function frame(t){if(state!=='playing')return;game.update((t-last)/1000,keys);last=t;updateHUD();draw();if(game.over){finish();return;}raf=requestAnimationFrame(frame);}
...
function pause(){if(state!=='playing')return;state='paused';keys={};cancelAnimationFrame(raf);$('#pause-game').textContent='Continuar';showOverlay('MISSÃO EM ESPERA','Pausa','Naves, tiros e dificuldade ficam congelados.','Continuar');updateHUD();}
function resume(){if(state!=='paused')return;state='playing';keys={};$('#game-overlay').hidden=true;$('#pause-game').textContent='Pausar';last=performance.now();updateHUD();canvas.focus({preventScroll:true});raf=requestAnimationFrame(frame);}
